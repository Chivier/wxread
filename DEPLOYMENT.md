...
计划北京时间每天 01:07、05:07、09:07、13:07、17:07、21:07 检查剩余时长，GitHub 排队可能延迟或漏掉单次调度。每次串行运行最多三个约两小时的 session；每个 job 有 200 分钟上限。前一段失败时，下一段仍会读取检查点尝试恢复；用户取消时停止后续段。重跑按北京时间当日累计计数，达到 `21600` 秒后停止。后续检查会在安装浏览器前直接跳过阅读。手动短测也计入当日累计。
